...
exports.createMeeting = async function (req, res) {
  // Validate a request
  const { isValid, message } = await validateCreateNewMeetingReq(req);
  ...
  if (meeting.length > 0) {
    return res
      .status(200)
      .json({ message: "event url already exist", eventURLExist: true });
  }
...
exports.meetingCheck = async function (req, res) {
  ...
  if (user.length < 1) {
    ...
    return res
      .status(200)
      .json({ message: "calendarURL doesn't exist", isExist: false });
  }
  ...
  if (meeting.length < 1) {
    ...
    return res
      .status(200)
      .json({ message: "eventURL doesn't exist", isExist: false });
  }
